...
* API: durable OpenLineage batch admissions now retain their request boundary through asynchronous projection. The worker can claim and project up to `openLineage.projectionBatchSize` events from one admission in a transaction, but never combines separate HTTP admissions. Per-lane FIFO and the durable q2 scheduling quantum still apply: a lane with `refresh_due_on_advance=false` can contribute its immediate follower from the same admission, while a lane with the bit set contributes only its head. The larger transaction can hold several queue and projection locks until commit; confirmed whole-transaction rollback restores the claimed subset without consuming attempts.
* API: `POST /api/v1/lineage/batch` accepts a bare array of 1–1000 OpenLineage events and returns `204` only after atomically committing the entire batch to the durable intake queue. Any invalid member rejects the whole batch. Projection remains asynchronous, same-lane events retain request order, and no order is promised across lanes. Any admission `5xx` is commit-indeterminate for the whole transaction, so retrying may duplicate every event in the batch.
...
* Database: durable intake requires Flyway migrations V77 through V81. Deployments with `migrateOnStartup: false` must run the current source's `db-migrate` command before starting the application.
* Database: the OpenLineage worker selects a bounded ready subset from one durable batch admission with `FOR UPDATE SKIP LOCKED` and performs projection plus acknowledgement, retry, or dead-letter transition in one READ COMMITTED transaction. Singular-endpoint and legacy rows remain singleton claims. Successful multi-event claims use bulk queue finalization. If their speculative projection fails ordinarily, the worker rolls back its batch savepoint and applies the existing per-event failure policy under savepoints in the same transaction; a retry blocks only later claimed work from that lane. The durable q2 scheduling bit refreshes the indexed due clock every second successful promotion. Confirmed whole-transaction rollbacks consume no `maxAttempts` attempt. A connection or process failure while the commit result is being returned is indeterminate: projection and acknowledgement may already be committed.
...
* Operations: OpenLineage worker shutdown uses two deadline-bounded executor waits. After graceful drain times out, Marquez interrupts remaining tasks, synchronously aborts every registered JDBC connection so its open transaction rolls back, and then performs the second wait. The synchronous driver abort calls between those waits have no application-enforced deadline. The `open-lineage-worker` health check reports fatal task or coordinator failures and persistent polling failure; committed retry and dead-letter outcomes remain healthy. The Helm Deployment uses `Recreate` to prevent old and new lineage projector replicas from overlapping, with brief API unavailability expected during rollout.
* Metrics: lease/claim/direct-handoff worker metrics have been replaced by the label-free Codahale suffixes `selected`, `succeeded`, `retried`, `dead_lettered`, `poll_failed`, `task_failed`, `coordinator_failed`, `state_transition_failed`, `post_commit_failed`, `batch_fallback`, `forced_shutdown`, `shutdown_incomplete`, `poll_duration`, `poll_empty`, `processing_duration`, `post_commit_duration`, `claim_size`, and `in_flight`; the existing `running`, `coordinator_alive`, `processor_capacity`, and `available_processor_capacity` gauges remain. `selected` can exceed committed outcomes after a lane retry leaves a claimed follower unprocessed, rollback, or process loss.
