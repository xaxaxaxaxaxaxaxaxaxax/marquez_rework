...
* Configuration: added `openLineage.projectionBatchSize` to bound the number of events from one admitted batch projected per transaction (default 8, valid range 1–64); set it to 1 to retain singleton projection.
* API: `POST /api/v1/lineage/batch` accepts a bare array of 1–1000 OpenLineage events and returns `204` only after atomically committing the entire batch to the durable intake queue. Any invalid member rejects the whole batch. Projection remains asynchronous and may occur in bounded subsets across multiple transactions, so atomic durable admission does not imply atomic projection visibility for the whole batch. Same-lane events retain request order, and no order is promised across lanes. Any admission `5xx` is commit-indeterminate for the whole transaction, so retrying may duplicate every event in the batch.
* API: `POST /api/v1/lineage` now returns `201` only after committing the event to a durable database queue. Lineage projection is asynchronous and the queue has no application-level event-count cap. Any admission `5xx` is commit-indeterminate: the queue transaction may already have committed even if Marquez did not observe the result, so clients that retry must tolerate duplicate events. Search indexing and run-transition listener delivery occur after queue acknowledgement and are best effort: they have no durable delivery guarantee, may be missing, concurrent, or out of order, and are not retried by the intake queue. The legacy `openLineage.queueCapacity` setting remains accepted for configuration compatibility but is ignored.
* API: durable OpenLineage intake requires `db.defaultTransactionIsolation: READ_COMMITTED`. An omitted or `DEFAULT` setting is normalized to `READ_COMMITTED`; an explicitly configured alternative fails startup.
* Database: automatic Jdbi DAO/method SQL comments now default to disabled. Set `db.autoCommentsEnabled: true` to retain `/* DAO.method */` prefixes in PostgreSQL logs and `pg_stat_statements`; SQL behavior is otherwise unchanged.
* Database: durable intake and projection require Flyway migrations V77 through V83. Deployments with `migrateOnStartup: false` must run the current source's `db-migrate` command before starting the application. V83 is deliberately forward-only: it creates `open_lineage_run_io_state` without backfilling historical runs, and an absent `(run, side)` row remains the signal to read that side from the legacy cumulative mappings.
* Database: the OpenLineage worker now projects a bounded subset from one durable batch admission per READ COMMITTED transaction through a batch-first projection kernel. The kernel canonicalizes the whole batch's alias graph, retains every event-owned fact, and applies mutable shared job and dataset snapshots by last-write-wins order `(UTC eventTime, SHA-256 of the exact queued JSON)`. It still produces each event's effective listener result. Singular-endpoint and legacy rows remain singleton claims; existing retry, dead-letter, rollback-attempt accounting, and indeterminate-commit semantics are preserved.
* API: queued RunEvent input and output lists are authoritative independently when that side is present. A missing side retains its prior value, a present empty list clears it, and a present non-empty list replaces it with the reported ordered occurrences. V83 stores each side's own LWW watermark and ordered dataset-version UUID array; sides with no V83 row continue to use the cumulative legacy fallback until a future event reports that side.
* Search: OpenSearch document IDs for OpenLineage jobs and datasets now use delimiter-safe, unpadded Base64URL components and the same canonical namespace as relational projection. Search-enabled deployments must recreate or reindex the `jobs` and `datasets` indexes after upgrading; otherwise legacy colon-delimited documents may coexist with the new IDs and produce duplicate or stale hits.
* Configuration: the obsolete `openLineage.leaseDurationMillis` setting and Helm value have been removed; an explicit compatibility guard rejects stale application YAML containing that property. It is not interpreted as a transaction timeout. This guard does not change the parser's general unknown-property behavior.
* Operations: OpenLineage worker shutdown uses two deadline-bounded executor waits. After graceful drain times out, Marquez interrupts remaining tasks, synchronously aborts every registered JDBC connection so its open transaction rolls back, and then performs the second wait. The synchronous driver abort calls between those waits have no application-enforced deadline. The `open-lineage-worker` health check reports fatal task or coordinator failures and persistent polling failure; committed retry and dead-letter outcomes remain healthy. The projector rework is a direct-replacement rollout: stop all old replicas, apply V83, and start the new replicas without overlapping projector versions. The Helm Deployment uses `Recreate` to enforce that boundary, with brief API unavailability expected during rollout.
* Metrics: lease/claim/direct-handoff worker metrics have been replaced by the label-free Codahale suffixes `selected`, `succeeded`, `retried`, `dead_lettered`, `poll_failed`, `task_failed`, `coordinator_failed`, `state_transition_failed`, `post_commit_failed`, `batch_fallback`, `forced_shutdown`, `shutdown_incomplete`, `poll_duration`, `poll_empty`, `processing_duration`, `post_commit_duration`, `claim_size`, and `in_flight`; the existing `running`, `coordinator_alive`, `processor_capacity`, and `available_processor_capacity` gauges remain. Selection precedes transaction completion, so `selected` can exceed the committed outcome meters.
* Operations: scheduled retention and the one-off `db-retention` command create or replace persistent PostgreSQL helper functions and therefore require DDL permission. `db-retention --dry-run` does not delete retained rows, but it still creates or replaces its separate persistent estimation helper.
