...
* Database: durable intake requires Flyway migrations V77 through V80. Deployments with `migrateOnStartup: false` must run the current source's `db-migrate` command before starting the application.
* Database: the OpenLineage worker now selects one due head with `FOR UPDATE SKIP LOCKED` and performs projection plus acknowledgement, retry, or dead-letter transition in the same READ COMMITTED transaction. The durable q2 scheduling bit refreshes the indexed due clock every second successful promotion. A caught projection failure rolls back to a savepoint and increments the persisted failure count only when its retry or dead-letter transition commits; confirmed whole-transaction rollbacks consume no `maxAttempts` attempt. A connection or process failure while the commit result is being returned is indeterminate: projection and acknowledgement may already be committed.
...
* Metrics: lease/claim/direct-handoff worker metrics have been replaced by the label-free Codahale suffixes `selected`, `succeeded`, `retried`, `dead_lettered`, `poll_failed`, `task_failed`, `coordinator_failed`, `state_transition_failed`, `post_commit_failed`, `forced_shutdown`, `shutdown_incomplete`, `poll_duration`, `poll_empty`, `processing_duration`, `post_commit_duration`, and `in_flight`; the existing `running`, `coordinator_alive`, `processor_capacity`, and `available_processor_capacity` gauges remain. `selected` can exceed committed outcomes after rollback or process loss.
