...
* Database: durable intake requires Flyway migrations V77 through V82. Deployments with `migrateOnStartup: false` must run the current source's `db-migrate` command before starting the application.
* Database: the OpenLineage worker now projects a bounded subset from one durable batch admission per READ COMMITTED transaction. Singular-endpoint and legacy rows remain singleton claims; existing retry, dead-letter, rollback-attempt accounting, and indeterminate-commit semantics are preserved.
